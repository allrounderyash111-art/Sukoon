...
/* ---------- SCENE SCALING ENGINE ----------
   Keeps the artwork (1376x768) filling the screen edge-to-edge on
   any device, while every animation inside it stays perfectly
   aligned because they all scale together as one unit. */
const scene = document.getElementById('scene');
function fitScene() {
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  const scale = Math.max(vw / 1376, vh / 768);
...
for (let i = 0; i < 26; i++) {
  ...
  s.className = 'twinkle';
  s.style.left = (Math.random() * 1376) + 'px';
...
const radioGlow = document.getElementById('radioGlow');
